...
class REG(IntEnum):
    ...
    REG_FRAME_MULT_INN      = 0x0035    # dec:53 - multiplier inner channel
    REG_FRAME_FLAGS_INN     = 0x0036    # dec:54 - inner spotlight frame
    REG_FRAME_STEP_INN      = 0x0037    # dec:55 - inner frame step
    REG_FRAME_WORKS_INN     = 0x0038    # dec:56 - inner index
    REG_AUTO_HEAT_MODE      = 0x0039    # dec:57 - set and get auto heat mode
    REG_FRAME_MULT_EXT      = 0x003A    # dec:58 - multiplier external channel
    REG_FRAME_FLAGS_EXT     = 0x003B    # dec:59 - external spotlight frame
    REG_FRAME_STEP_EXT      = 0x003C    # dec:60 - external frame step
    REG_FRAME_WORKS_EXT     = 0x003D    # dec:61 - external index
...
class Default_Value(IntEnum):
    ...
    DEF_FRAME_INN_MULT      = 1         # count to multiply input pulse of the inner channel
    DEF_FRAME_EXT_MULT      = 1         # count to multiply input pulse of the external channel
    DEF_AUTO_HEAT_MODE      = 0         # see heat_mode_t in heat.h
...
cmd_read_array = {
    ...
    "frame_mult": {
        "type"          : TYPE.READ,
        "description"   : 'Чтение параметров умножения тактового сигнала для прожектора (и внутренний и внешний).',
        "modbus": {
            "address"   : DEVICE_ADDR,
            "function"  : FUNC.MB_FC3_READ_REGISTERS,
            "register"  : REG.REG_FRAME_MULT_INN,
            "value"     : 0x0006
        }
    },
    ...
    "frame_state": {
        "type"          : TYPE.READ,
        "description"   : 'Чтение состояния в процессе работы тактовых сигналов для прожектора.',
        "modbus": {
            "address"   : DEVICE_ADDR,
            "function"  : FUNC.MB_FC3_READ_REGISTERS,
            "register"  : REG.REG_FRAME_MULT_INN,
...
cmd_write_array = {
    ...
    "cam_filter": {
        "type"          : TYPE.WRITE,
        "description"   : "Управление мотором фильтра камеры",
        "modbus": {
            "address"   : DEVICE_ADDR,
            "function"  : FUNC.MB_FC6_WRITE_REGISTER,
            "register"  : REG.REG_IRF,
            "value"     : 0x0000
        }
    },
    ...
    "frame_inn_mult": {
        "type"          : TYPE.WRITE,
        "title"         : "[WRITED] Frame Multiply of the Spotlight (config), Inner Channel",
        "units"         : "-",
        "description"   : 'Запись параметра умножения тактового сигнала для прожектора (внутренний канал).',
        "modbus": {
            "address"   : DEVICE_ADDR,
            "function"  : FUNC.MB_FC6_WRITE_REGISTER,
            "register"  : REG.REG_FRAME_MULT_INN,
            "value"     : Default_Value.DEF_FRAME_INN_MULT
        }
    },

    "frame_ext_mult": {
        "type"          : TYPE.WRITE,
        "title"         : "[WRITED] Frame Multiply of the Spotlight (config), External Channel",
        "units"         : "-",
        "description"   : 'Запись параметра умножения тактового сигнала для прожектора (внешний канал).',
        "modbus": {
            "address"   : DEVICE_ADDR,
            "function"  : FUNC.MB_FC6_WRITE_REGISTER,
            "register"  : REG.REG_FRAME_MULT_EXT,
            "value"     : Default_Value.DEF_FRAME_EXT_MULT
        }
    },
    ...
    "autoheat_mode": {
        ...
        "modbus": {
            ...
            "function"  : FUNC.MB_FC6_WRITE_REGISTER,
            "register"  : REG.REG_AUTO_HEAT_MODE,
            "value"     : Default_Value.DEF_AUTO_HEAT_MODE
        }
    },
    #############
}
